...
class EncoderWrapper(width: Int,
    ...
    private val FrameTimeStampList = arrayListOf<Any>()

    public fun GetFrameTimeStampList() : ArrayList<Any> {
        //record still running
        if(mEncoderThread.isAlive) return arrayListOf<Any>(0L)

        return this.FrameTimeStampList
    }
    ...
    public fun frameAvailable() {
        val handler = mEncoderThread.getHandler()
        FrameTimeStampList.add(System.currentTimeMillis())
